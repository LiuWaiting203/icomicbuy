        ...
        VTable.tableFix
          ...
          tbody
            tr(v-for="order in orders" :key="order._id")
              td.text-center.text-caption.border-table-R.text-body-2 {{ dateString(order.date) + numSplit(order._id) }}
              td.text-center.border-table-R.text-body-2 {{ new Date(order.date).toLocaleString() }}
              td.text-center.border-table-R.text-pink ${{ order.total }}
